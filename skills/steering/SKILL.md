...
5. **ユーザー確認（必須）**
   ...
   ステアリングファイル（requirements.md / design.md / tasklist.md）の作成が完了したら、**実装（モード2）に進む前に必ずユーザーに確認を依頼**する。

   例:
   ```
   「ステアリングファイル（requirements.md / design.md / tasklist.md）を作成しました。内容をご確認ください。OKなら実装（モード2）に進みます。修正点があれば指示ください。」
   ```

   **ユーザーのOK（または修正指示の反映）が完了するまでモード2に進まない**。
